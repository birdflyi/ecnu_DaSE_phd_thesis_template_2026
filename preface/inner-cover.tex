\pagestyle{empty}
\setlength{\baselineskip}{25pt}  %%正文设为25磅行间距
\vspace{-2.0cm}
\noindent{{\zihao{4} {\large \year} 届研究生{\degreeCN}学位论文}}\\
\vspace{-0.8cm}
\begin{flushleft}
\hspace{-0.5cm}
\renewcommand\arraystretch{1.5}
\begin{tabular}{l}
    \noindent{{\zihao{4} 分类号：\underline{~~~\qquad\qquad\qquad\qquad}}}   \\
    \noindent{{\zihao{4} 密~~~~级：\underline{~~~\qquad\qquad\qquad\qquad}}} \\
\end{tabular}
\hskip 1.1cm
\renewcommand\arraystretch{1.5}
\begin{tabular}{l}
    \noindent{{\zihao{4} 学校代码：\underline{~~~\qquad 10269 \qquad~~~}}}   \\  %10269
    \noindent{{\zihao{4} 学~~~~~~~~号：\underline{~~~~~~{\stuID}~~~~~}}} \\
\end{tabular}
\end{flushleft}


\vskip 1.3cm

\begin{center}
% \hskip 0.2cm
% \scalebox{1.0}{\includegraphics[width=2.7cm]{chapters/fig-0/ecnulogo.png}}
\scalebox{1.0}{\includegraphics[width=13.2cm]{chapters/fig-0/logo.png}}
\vskip 0.5cm
{{\erhao East China Normal University}}\\ 
{\textbf{\erhao {\degreeCN}学位论文}}\\ 
{\textbf{{\erhao \MakeUppercase{\degreeENs} DISSERTATION}}}\\
\end{center}

\vskip 0.8cm % 1.0cm

\begin{center}
{\erhao \bf {\thesisTitle}}
% {\cyihao \bf 论文题目：\underline{论文格式研究与} \par }
% {\cyihao \bf \underline{更长的标题编写} \par }
%{\erhao \bf ~~~~\underline{}
\end{center}

\newcommand{\coverlength}{2.7cm}
\vskip 1.0cm 
\begin{center}
    \renewcommand\arraystretch{1.5}
    \begin{tabular}{l}
        \makebox[\coverlength][s]{\sihao \bf 培养单位:}~~\\
        \makebox[\coverlength][s]{\sihao \bf 学科:}\\ 
        \makebox[\coverlength][s]{\sihao \bf 研究方向:}\\
        \makebox[\coverlength][s]{\sihao \bf 指导教师:}\\ 
        \makebox[\coverlength][s]{\sihao \bf 学位申请人:}
    \end{tabular}
    \begin{tabular}c
        {\sihao \bf ~~~数据科学与工程学院~~~}\\ 
        \hline {\sihao \bf 软件工程(数据科学与工程)}\\ 
        \hline {\sihao \bf 计算机视觉}\\ 
        \hline {\sihao \bf \ifnotanonymous ~~李四~~教授, 王五~~教授~~ \else *** \fi}\\
        \hline{\sihao \bf  \ifnotanonymous ~~张三~~ \else *** \fi}\\
        \hline
    \end{tabular}
\end{center}

\vskip 3.5cm 

\begin{center}
    {\sihao {\year}年3月20日}
\end{center}
